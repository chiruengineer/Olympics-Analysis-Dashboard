...
export async function GET() {
  try {
    ...
    records.forEach((record: any) => {
      ...
      if (record.year) countryData.years.add(Number.parseInt(record.year))
      if (record.sport) countryData.sports.add(record.sport)
    })
    ...
    const countries = Array.from(countryMap.values())
      .map((country) => ({
        ...country,
        years: Array.from(country.years).sort(),
        sports: Array.from(country.sports).sort(),
      }))
