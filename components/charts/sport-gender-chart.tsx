...
interface SportGenderChartProps {
  sports: SportData[]
}

export function SportGenderChart({ sports }: SportGenderChartProps) {
  const chartData = sports.map((sport) => ({
    sport: sport.sport.length > 12 ? sport.sport.substring(0, 12) + "..." : sport.sport,
    fullSport: sport.sport,
    male: sport.maleParticipation,
    female: sport.femaleParticipation,
  }))
